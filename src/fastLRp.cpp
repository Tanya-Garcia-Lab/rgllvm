...
public:
  ...
  double f_grad(Constvec& beta, Refvec grad)
  {
    ...
    for(int i = 0; i < n; i++)
     ...
     const double f = prob.sum() - yxbeta + 100*beta.dot(beta);
    // const double f = prob.sum() - yxbeta;
    // Gradient
    //   X' * (p - y), p = exp(X * beta) / (1 + exp(X * beta))
    //                   = exp(X * beta - log(1 + exp(X * beta)))
    prob = (xbeta - prob).array().exp();
    //grad.noalias() = X.transpose() * (Y - prob) - 2*0.1*beta;
    //grad.noalias() = X.transpose() * (Y - prob);
    //grad +=-2*0.1*beta;
...
// [[Rcpp::export]]
Rcpp::List fastLRp_(Rcpp::NumericMatrix x, Rcpp::NumericVector y,
